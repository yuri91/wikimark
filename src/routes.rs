...
use askama::Template;
use std::sync::Arc;
use super::{errors, git, templates, page, WikiState};
...
impl Header for User {
        ...
        I: Iterator<Item = &'i HeaderValue>
    {
        ...
        Ok(User(value.to_str().map_err(|_| axum::headers::Error::invalid())?.to_owned()))
    }
...
pub async fn pages(State(state): State<Arc<WikiState>>, user: Option<UserHeader>) -> Result<Html<String>> {
    let repo = state.repo.lock().expect("error aquiring mutex");
    let user_str = user.as_ref().map(|u| u.0.0.as_str());
...
pub async fn commit(State(state): State<Arc<WikiState>>, user: UserHeader, Json(info): Json<git::CommitInfo>) -> Result<Html<String>> {
    let repo = state.repo.lock().expect("error aquiring mutex");
