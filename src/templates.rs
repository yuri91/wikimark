...
pub fn index() -> impl Fn(Arc<state::State>) -> String + Clone {
    move |state| {
        ...
        state.tera.render("index.html", &ctx).expect("template rendering failed")
    }
}
...
pub fn page() -> impl Fn(Arc<state::State>, String) -> String + Clone {
    move |state, mut fname| {
        ...
        state.tera.render("page.html", &ctx).expect("template rendering failed")
    }
}
...
pub fn all() -> impl Fn(Arc<state::State>) -> String + Clone {
    move |state| {
        let repo = git::get_repo("repo");
        let list = git::list_files("", &repo).into_iter().filter_map(|f| {
            if f.ends_with(".md") {
